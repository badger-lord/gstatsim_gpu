...
# CUDA kernel code with proper INFINITY and NAN definitions
...
// Chebyshev coefficients from your original implementation
...
// Device constant for intervals (default 128)
...
# Global variables for lazy compilation
...
def kv_gpu(nu, x, scaled=False):
    ...
    This implementation uses the sophisticated algorithms from the original
    C code including Chebyshev polynomials and Temme's method.
    ...
    --------
    cupy.ndarray
        Values of K_nu(x), optionally scaled by exp(x)
    """
...
# Compatibility functions matching the C interface
def setIntervals(nbins):
    """
    Set the number of integration intervals for large-x computation.
    ...
    """
    # Note: In this implementation, intervals is a compile-time constant
    # This function maintains API compatibility but doesn't change runtime behavior
    pass

def initBesselK():
    """Initialize the Bessel K computation with default parameters."""
    setIntervals(128)

def BesselK_CUDA(host_x, host_v, host_result, n):
    """
    Compute Bessel K function with C-compatible interface.
    ...
    if not isinstance(host_x, cp.ndarray):
        x_gpu = cp.asarray(host_x)
    else:
        x_gpu = host_x
        
    if not isinstance(host_v, cp.ndarray):
        v_gpu = cp.asarray(host_v)
    else:
        v_gpu = host_v
