...
# -----------------------------------------------------------------------------
def _pairwise_distance_squared_matmul(X, Y):
    """
...
def batch_ok_solve_gpu(sim_points, neighbors_array, vario, jitter_rel=1e-6):
    """
    ...
    dist_sq = _pairwise_distance_squared_matmul(coords_r, coords_r)
    NR = cp.sqrt(dist_sq)
    ...
    dist_sq_sim = _pairwise_distance_squared_matmul(coords_r, sim_r[:, None, :])
    ...
    b[:, :K] = rho
    b[:, K] = 1.0 # Constraint sum(weights) = 1
...
def _solve_group_sk(sim_points, neighbors_array, vario, global_mean):
    ...
    Sigma = batch_covariance_gpu(NR, vtype, sill, nugget, s=s)
    
    # Regularization
    eps = 1e-6 * (cp.max(cp.abs(Sigma)) + 1.0)
    Sigma = Sigma + cp.eye(K)[None,:,:] * eps
    
    # Compute rho (RHS)
    sim_r = sim_points @ rot.T
    dist_sq_sim = _pairwise_distance_squared_matmul(coords_r, sim_r[:, None, :])
    nr = cp.sqrt(dist_sq_sim[:, :, 0])
    
    rho = batch_covariance_gpu(nr, vtype, sill, nugget, s=s)
